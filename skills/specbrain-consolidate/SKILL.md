...
- If there's at least one `feature` task: call `mcp__specbrain__record_indicator` with `key="demand_rework_rate"`, `value={"bug_count": <n>, "feature_count": <n>}`, `source="consolidate"`, `metadata={"design_id": "<id>"}` — the ratio itself (`bug_count / feature_count`) is derived by whoever reads this later (e.g. Admin Web), not computed here. The name says what it measures: **rework inside the demand**, not defects that escaped it. (It was called `demand_defect_rate` until the taxonomy pass; historical rows keep the old key and Admin Web groups them together.)
- Call `mcp__specbrain__list_defects` with this design's `design_id` and record `key="escaped_defect_rate"`, `value={"production": <n>, "ci": <n>, "qa": <n>, "review": <n>, "feature_count": <n>}`, `source="consolidate"`, `metadata={"design_id": "<id>"}`. Only `production` means the pipeline shipped something wrong; the other origins mean something caught it first, which is the system working. Record zeros when there are no defects — a demand that escaped clean is a reading, not a missing one.

### Step 4: Re-evaluate indicators for process insight

1. Call `mcp__specbrain__list_indicators` (no `key` filter) for this project.
2. Group the results by `(key, source)` and look at how each has trended. Every indicator belongs to one of three classes, and only one of them is worth reasoning about here:
   - **cost** (`retrieval_tokens`, `tokens_per_artifact`) and **throughput** (`task_cycle_time`, `orchestration_summary`, `task_kind_distribution`) — report them, don't theorize about them. They say what something cost or how fast it moved, not whether it was right.
   - **calibration** — whether the pipeline's own judgment matches reality: `llm_review_pass_rate`, `review_pass_rate`, `review_miss`, `review_panel_grounding_rate`, `demand_rework_rate`, `escaped_defect_rate`, `refine_root_cause`. This is the class that produces directives.

   Readings that were retired are still in the database and still visible in Admin Web, labelled as historical: `gate_pass_rate` (from when an LLM verdict could block a merge) and `rag_token_savings` (never a measurement — its baseline was every learning in the project, so it rose as the memory got worse). Don't reason from them; `record_indicator` refuses to write either one.

   The pairing worth reading closely is `llm_review_pass_rate` against `escaped_defect_rate`: a reviewer that approves everything while defects reach production is measuring nothing, and that is a directive on `orchestrate.gate`, not a comment about the implementers.
3. Reason about what the numbers actually suggest, not just what they say — e.g. a review that has never once failed anything might be too lenient rather than genuinely reflecting flawless work; a high `demand_rework_rate` on a design suggests its spec/discovery was thin going in. Only surface something if it's a real, non-obvious observation — with too little data, say that plainly instead of inventing an insight.
4. **An insight is only actionable if it names the stage it should steer.** For each observation, decide which pipeline stage would have to behave differently for it to stop recurring — `discovery`, `design`, `engineering.spec`, `engineering.review`, `engineering.tasks`, `orchestrate.implement`, `orchestrate.gate`, `review`, `refine`, `consolidate` or `quick`. An observation that maps to no stage is reported to the user and goes no further: it is not saved as a directive, and it is not saved as prose either. That is the rule that keeps this from silting up the way `process-insight` learnings did.
   ...
   Most calibration indicators already carry the stage they point at, so use that rather than re-deriving it:

   | Indicator | Steers |
   |---|---|
   | `refine_root_cause = context_gap` | `discovery` |
   | `refine_root_cause = design_gap` | `engineering.spec` |
   | `refine_root_cause = implementation_slip` | `orchestrate.implement` |
   | `demand_rework_rate` high | `engineering.spec` |
   | `escaped_defect_rate` (production) high | `review` |
   | `llm_review_pass_rate` ~100% with escapes | `orchestrate.gate` |
   | `review_miss` recurring | `review` |
   | `review_panel_grounding_rate` low | `engineering.review` |

   `refine_root_cause` is the single most directly actionable reading here: it names a real failure and the stage that let it through, in the same row.
...
- [ ] Computed `task_kind_distribution`, `demand_rework_rate` (rework inside the demand) and `escaped_defect_rate` (what got out, by origin) for the closed design, spanning every task and defect across all rounds
- [ ] Reasoned only from calibration indicators; reported cost/throughput without theorizing, and ignored the retired keys
