...
6. **Record indicators for this round.** For each lens dispatched this round, call `mcp__specbrain__record_indicator` with `key="review_panel_grounding_rate"`, `source="<lens name, e.g. business>"`, `value={"grounded": <n>, "ungrounded": <n>}` (counts among that lens's findings this round). The round's own totals (`findings_total`/`findings_actioned`/`findings_dismissed`) go in the `gate_round` artifact's `metadata` from the previous step, not into an indicator. They describe one round of one demand — that is an artifact's job. `review_panel_summary` used to be recorded as an indicator and is retired; `record_indicator` refuses it.
...
- [ ] Recorded `review_panel_grounding_rate` per lens dispatched, and kept each round's finding totals in the `gate_round` artifact rather than as an indicator
