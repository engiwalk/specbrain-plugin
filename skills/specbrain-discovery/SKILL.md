...
For each one the user answers now: fold the answer into the demand's existing context with `mcp__specbrain__update_artifact` on that inquiry's `parent_id` — the full corrected `content`, plus a `reason` naming the question that was answered. The previous text is kept as a revision, and the demand's context stays a single artifact that reads as current truth rather than a chain a future reader has to reassemble in the right order. Then call `mcp__specbrain__update_artifact_status` on the inquiry with `status="answered"`. Evaluate the answer against the same reusable-learning criteria as Step 9 below — if it's a durable business rule, `save_learning` it now rather than waiting.
