...
Run `"${CLAUDE_PLUGIN_ROOT}/scripts/specbrain-worktree.sh" cleanup-check`. If it fails, the working tree is dirty — tell the user to commit or stash before running this skill.
...
Call `mcp__specbrain__plan_waves` with the chosen `design_id`. It returns the tasks grouped into waves — every task in a wave can run in parallel; each wave waits for the one before it — with each task's content and acceptance criteria already attached.

Dependency resolution, wave computation, cycle detection and already-finished dependencies are all handled there. Do not re-derive any of it here: each has exactly one correct answer, and an answer re-derived in prose drifts silently across model versions.

If the response is `{"refused": "dependency_cycle"}` or `{"refused": "unknown_dependency"}`, stop and report the reason to the user verbatim. **Nothing has been created on disk yet, and that is the point of planning first.**

### Step 3: Create the integration worktree

Every git operation in this skill goes through the plugin's worktree script, at `${CLAUDE_PLUGIN_ROOT}/scripts/specbrain-worktree.sh`. It refuses rather than improvises, and it never touches the repository's main worktree — which is what lets two orchestration runs (different demands, or different sessions) coexist on the same repository.

```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/specbrain-worktree.sh" cleanup-check
"${CLAUDE_PLUGIN_ROOT}/scripts/specbrain-worktree.sh" create-integration <design-short>
```

`<design-short>` = the first 8 characters of the design's `id`. The script prints the integration worktree's path; keep it for Step 4's checks. It cuts the branch from `main` (or `master`) without ever checking that branch out.

### Step 4: Execute each wave, in order

For the current wave:

1. **Create worktrees (controller, sequential).** For each task in the wave, run `"${CLAUDE_PLUGIN_ROOT}/scripts/specbrain-worktree.sh" create-task <design-short> <task-short>` (`<task-short>` comes back as `short` from `plan_waves`). One at a time, never in parallel: two `git worktree add` calls racing on the same `.git` is a corrupted repository, not a slow one. Then call `mcp__specbrain__update_artifact_status` with that task's `id` and `status="in_progress"`.
...
7. **Merge when the verification is green:** run `"${CLAUDE_PLUGIN_ROOT}/scripts/specbrain-worktree.sh" merge-task <design-short> <task-short>`, which merges `--no-ff` and removes the task worktree. If the merge doesn't apply cleanly the script refuses and leaves the worktree in place — treat that like a failed task. Then mark the task `done` via `update_artifact_status` (the server accepts it only because a passing verification exists). If that comes back `{"refused": "verification_required"}`, something is out of order — report it rather than working around it.
8. **Verify the integration branch itself, once per wave.** After the wave's merges, run every mandatory check from `<checks>` inside the integration worktree (`specbrain-worktree.sh integration-path <design-short>`) — in full, regardless of `<test-scope>` — and record the result with `mcp__specbrain__record_verification` using `scope="integration"`, `integration_branch="specbrain/<design-short>"` and no `task_id`. If anything is red, **stop the wave chain here**: report which check failed and its output, and do not start the next wave. This is the only place a merge-interaction failure can surface — each task's own checks passed in isolation by construction, so two green tasks can still be red together.

After every task in the wave has reached `done` or `failed`: call `mcp__specbrain__cascade_blocked` with the ids of everything that failed so far, and mark each returned task `blocked` via `update_artifact_status`. It resolves the transitive chain, so a task blocked three levels down comes back from the same call — don't walk `depends_on` by hand.
...
- [ ] Planned the waves with `plan_waves` — never re-derived the graph here — and stopped on a refusal before anything was created on disk
- [ ] Created the integration worktree through the plugin's worktree script, never with hand-written git commands
...
- [ ] Used `cascade_blocked` to find the dependents of failed tasks, rather than walking `depends_on` by hand
